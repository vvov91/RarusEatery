...
public class DBManager extends SQLiteOpenHelper {
	...
	public void addMenu(List<RarusMenu> menu) {
		...
		try {
			for (int i = 0; i < menu.size(); i++) {
				...
				insertMenuStmt.bindString(2, Integer.toString(menu.get(i).getDate()));
		...
		try {
			Cursor c = mDb.query(false, query.toString(),
					new String[] {"MU." + KEY_ID, MENU_DATE, "DS." + KEY_ID, DISHES_NAME,
						...
						MENU_DATE + " = ?", new String[] {Integer.toString(date)},
						null, null, null, null);
			mDb.setTransactionSuccessful();
	...
	public void deleteMenuAtDate(int date) {
		...
		try {
			...
			Log.i(LOG_TAG, "[DB] Deleted menu at date " + Integer.toString(date));
		}
	}
